...
<body class="bg-gray-50 p-10">
    <div class="max-w-xl mx-auto bg-white p-6 rounded-xl shadow">
        ...
        <form action="{{ route('body.update', $body->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <input type="text" name="judul" value="{{ old('judul', $body->judul) }}" class="w-full p-2 border rounded mb-4">

            <label class="block mb-2 font-semibold">Deskripsi</label>
            <textarea name="content" class="w-full p-2 border rounded mb-4" rows="4">{{ old('content', $body->content) }}</textarea>

            <label class="block mb-2 font-semibold">Gambar</label>
            @if($body->image)
                <img src="{{ asset('storage/'.$body->image) }}" alt="Preview" class="w-40 rounded mb-3">
            @endif
